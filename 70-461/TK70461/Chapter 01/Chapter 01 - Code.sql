...
SELECT custid, max(count(orderid)) as orders_counter
FROM Sales.Orders
GROUP BY custid
...
with a as (
	SELECT 
	   custid
	  , count(orderid) over(partition by custid order by orderid) as licz2
	FROM Sales.Orders) 
select custid, max (licz2) from a 


with a as (
	SELECT 
		 custid
		,count(orderid) as licz
	FROM Sales.Orders as a
	group by custid) 
select max(licz) from a 