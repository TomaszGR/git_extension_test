...
--tg
IF OBJECT_ID ('dbo.Large_objects') IS NOT NULL 
	DROP TABLE dbo.Large_objects;
go

SELECT
ROW_NUMBER() OVER(ORDER BY (SELECT 0)) as [orderid]
, so1.[custid], so1.[empid], so1.[orderdate], so1.[requireddate], so1.[shippeddate]
, so1.[shipperid], so1.[freight], so1.[shipname], so1.[shipaddress], so1.[shipcity]
, so1.[shipregion], so1.[shippostalcode], so1.[shipcountry]
INTO dbo.Large_objects
FROM Sales.Orders as so1
	CROSS JOIN Sales.Orders as so2

WHILE 1=1
BEGIN
	DELETE TOP (5000) FROM dbo.Large_objects
	IF @@ROWCOUNT < 5000
	BREAK 
END

SELECT COUNT(*) FROM dbo.Large_objects;
--(688900 rows affected)
...
CREATE FUNCTION dbo.GetTopProducts (@supid int, @catid as int, @n int)
...
SELECT * FROM dbo.GetTopProducts (1,1,100)

	SELECT ps.[supplierid], ps.[companyname], ps.[contactname], ps.[contacttitle], gtp.[productname], gtp.[unitprice]
	FROM [Production].Suppliers as ps
	CROSS APPLY dbo.GetTopProducts (ps.[supplierid], 1,2) as gtp
...
WITH cte as (
	SELECT 
	ROW_NUMBER() OVER(partition by [empid] order by [orderdate], [orderid] desc) as rn
...
--tg
SELECT he.[empid], so.[orderid], so.[custid], so.[orderdate], so.requireddate
...
WITH cte as (
	SELECT 
	ROW_NUMBER() OVER(partition by [empid] order by [orderdate] desc , [orderid] desc) as rn
	,[orderid], [custid], [empid], [orderdate]
	FROM Sales.Orders
	)
SELECT * FROM CTE WHERE rn <= 3;
...
SELECT so.empid, so.orderid, orn.orderid, so.orderdate, so.requireddate
FROM OrdersRN as so
	LEFT JOIN OrdersRN as orn
		ON orn.empid = so.empid
		AND orn.rn = so.rn + 1;
