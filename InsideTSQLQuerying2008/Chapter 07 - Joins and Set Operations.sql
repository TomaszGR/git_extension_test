...
---generwanie danych testowych dla ka¿dego klienta i ka¿dego pracownika na kazdy dzieñs stycznia 2009
...
GO 
	...
	SELECT ROW_NUMBER() OVER(ORDER BY [empid]) as orderid
	,[custid]
	,[empid]
	, '2009-01-'+right(replicate('0',2) + cast(N.n as varchar),2) as orderdate
...
,[custid]
,[empid]
, DATEADD(day,N.n-1,'2009-01-01') as orderdate
...
WHERE NOT EXISTS 
	(SELECT keycol
	 FROM dbo.T1 b
	 WHERE a.keycol + 1 = b.keycol )


SELECT * --MIN(a.keycol) + 1
FROM dbo.T1 AS a
LEFT JOIN dbo.T1 b ON a.keycol + 1= b.keycol
...
arrid
, array
, LEN(array) - LEN(REPLACE(array, ',', '')) as comma 
--, STRING_SPLIT(array,',') --2016
FROM dbo.Arrays
...
, array
, LEN(array) - LEN(REPLACE(array, ',', '')) as comma  
, ROW_NUMBER() OVER(partition by arrid order by arrid) as rn
, CHARINDEX (',',array)
, DATALENGTH(array)
FROM dbo.Arrays
...
SELECT arrid, array, n, DATALENGTH(array), SUBSTRING(array, n, 1)
FROM dbo.Arrays
  JOIN dbo.Nums
...
, DATALENGTH(array)
, SUBSTRING(',' + array, n, 1)
, SUBSTRING(array,n,DATALENGTH(array))
, CHARINDEX(',', array + ',', n) - n
FROM dbo.Arrays
  JOIN dbo.Nums
