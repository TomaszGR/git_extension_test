...
BEGIN
	DROP PROCEDURE dbo.GetCustOrders;
END;
GO

CREATE PROCEDURE dbo.GetCustOrders 
	  @custid	int
	, @fromdate datetime = '20000505' 
	, @todate	datetime = '22220505'
	, @numrows	int OUTPUT
AS
BEGIN
	SET NOCOUNT ON;
...
-- EXEC 
DECLARE @myproc as int ;
DECLARE @mynumrows as int;
--EXEC dbo.GetCustOrders @custid=1, @fromdate='20000505', @todate='20000505', @numrows= @mynumrows OUTPUT 
EXEC @myproc= dbo.GetCustOrders @custid=1, @fromdate='20000505', @todate='20000505', @numrows= @mynumrows OUTPUT 
SELECT @mynumrows 
...
CREATE TABLE tempdb.#CustOrders(
  orderid   INT,
  custid INT,
  empid   INT,
  orderdate  DATETIME)
...
IF OBJECT_ID ('dbo.GetSortedShippers') IS NOT NULL
	DROP PROC dbo.GetSortedShippers;
GO

CREATE PROCEDURE dbo.GetSortedShippers @colname nvarchar(100), @sortdir char(1) = N'D'
AS
BEGIN
	SET NOCOUNT ON;
	...
	IF @colname NOT IN (N'shipperid', N'companyname', N'phone')
	BEGIN
		RAISERROR ('Injection is possibly', 16, 1);  
	END;
    ...
    + @colname
	+ ' '
	+ CASE @sortdir WHEN 'D' THEN 'DESC' ELSE '' END
	+ ';';
