...
--tg20180421 0753
DECLARE @idstring as varchar(255)
set @idstring = ''

SELECT @idstring = @idstring + cast([orderid] as varchar (255))
FROM [InsideTSQL2008].[Sales].[Orders]
where custid = 85

  print @idstring

ALTER FUNCTION dbo.ConcatOrders 
	(
		@idcust as int
	)
RETURNS VARCHAR(MAX)
...
IF OBJECT_ID ('dbo.CustOrders') IS NOT NULL
BEGIN
	DROP FUNCTION dbo.CustOrders
END;
go

CREATE FUNCTION dbo.CustOrders (@custid as int)
RETURNS TABLE
AS
