...
WITH EmpMax AS (
	SELECT MAX(orderid) as maxoid, empid
	FROM Sales.Orders
	GROUP BY empid
	)
SELECT so.*
...
ALTER TABLE Sales.Customers alter column custid int not null;
...
ALTER TABLE Sales.Orders  alter column orderid int not null;
